...
const withFlags = codes.map(c => {
  const flag = String.fromCodePoint(...c.code.toUpperCase().split('').map(char => 127397 + char.charCodeAt(0)));
  return { ...c, flag };
});

const content = `export const COUNTRY_DIAL_CODES = ${JSON.stringify(withFlags, null, 2)};\n`;
fs.writeFileSync('src/countryCodes.ts', content);
